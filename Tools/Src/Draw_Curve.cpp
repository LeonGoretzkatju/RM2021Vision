...
//    bei = 1;
    ...
    for(int i = (Times + 1)%SIN_POINT_NUM;i<=Times&&i<SIN_POINT_NUM;i++){
        points.push_back(cv::Point2i ((float)image.cols/SIN_POINT_NUM*t ,image.rows/2-bei*SavePoint[i]));
        t++;
    }
    //绘制折线
//        cv::polylines(image, points, false, cv::Scalar(0, 255, 0), 1, 8, 0);
    ...
    t = 0;
    for(int i = (Times + 1)%SIN_POINT_NUM;i<=Times&&i<SIN_POINT_NUM;i++){
        SecPoints.push_back(cv::Point2i((float)image.cols/SIN_POINT_NUM*t ,image.rows/2-bei*SecSavePoint[i]));
...
//    imshow("波形图2",image2);
    ...
    if(key == 'p'){
        imwrite("/home/nuc/滤波后pitch波形图.jpg",image);
    }
    Times++;
}
